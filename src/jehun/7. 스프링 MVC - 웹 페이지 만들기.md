...
**전체 흐름**  
![img.png](img/session7/img6.png)  
클라이언트는 처음 상품 목록에서
1. 상품 등록
2. 상품 상세

2가지 선택지가 있으며, 등록할 때는 등록폼을 통해 상품 저장을 하면 내부 호출을 하는데  
**웹 브라우버의 새로 고침은 마지막에 서버에 전송한 데이터를 다시 전송한다.**  
등록 폼에서 데이터를 입력하고 저장을 하면 `POST /add` + 상품 데이터를 서버로 전송한다.  
새로 고침을 해도 또 POST /add + 상품 데이터를 서버로 전송한다.  
**즉, 내용만 같고, ID만 다른 상품 데이터가 계속 쌓인다.**  

![img.png](img/session7/img7.png)
...
**BasicItemController에 추가**
```java
    @PostMapping("/add") // redirect로 /basic/items/{id}로 GET 으로 가기 때문에 새로고침을 해도 조회만 반복한다.
