...
웹 브라우저 요청 -> 컨트롤러 호출 -> ResponseBody에 있는 내용을 전환해주는 HttpMessageConverter 작동 -> 리턴
...
![img.png](img/session6/img16.png)

위의 사진에는 보이지 않는다.  
애노테이션 기반의 컨트롤러, `@RequestMapping`을 처리하는 핸들러 어댑터인 `RequestMappingHandlerAdapter (요청 매핑 핸들러 어댑터)에 있다.  

**RequestMappingHandlerAdapter 동작 방식**  
![img.png](img/session6/img17.png)

컨트롤러에서 보면 각 컨트롤러마다 그에 맞는 파라미터 데이터가 필요하다.  
ex) `RequestBodyStringController`의 `V2`에서 `InputStream` 데이터, `HttpServletRequest` 데이터 등

이러한 매개변수들을 처리해주는 **ArgumentResolver**가 있다.
...
애노테이션 기반 컨트롤러를 처리하는 **`RequestMappingHandlerAdapter` 는 바로 이 `ArgumentResolver` 를 호출해서 컨트롤러(핸들러)가 필요로 하는 다양한 파라미터의 값(객체)을 생성**한다.  
...
```
**동작 방식**  
`ArgumentResolver` 의 **`supportsParameter()` 를 호출해서 해당 파라미터를 지원하는지 체크**하고, 지원하면 **`resolveArgument()` 를 호출해서 실제 객체를 생성**한다.  
...
**HTTP 메시지 컨버터의 위치**  
![img.png](img/session6/img18.png)
